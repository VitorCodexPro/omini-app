(function initApiLayer() {
  // Camada única de comunicação HTTP com tratamento padronizado de erros.
  const API = {
    ...
    async request(method, endpoint, body) {
      try {
        ...
        const payload = await response
          .json()
          .catch(() => ({ data: null, error: 'Resposta inválida do servidor.' }));

        if (!response.ok) {
          return {
            data: null,
            error: payload.error || `Erro HTTP ${response.status}`
          };
        }
  ...
  const OrcamentosAPI = {
    ...
    atualizarStatus: (id, status) =>
      API.patch(`/api/orcamentos?id=${encodeURIComponent(id)}`, { status }),
